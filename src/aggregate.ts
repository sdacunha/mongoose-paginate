import { PipelineStage, Aggregate, Callback } from "mongoose"
...
export function execPaged(
  this: Aggregate<unknown>,
  paginationOptions: PaginationOptions,
  callback?: Callback<PaginationResult<unknown> | null> | undefined
): void | Promise<PaginationResult<unknown>> {
  ...
  newPipeline.facet({
    results: [...(shouldSkip ? [{ $match: match }] : []), { $sort: sort }, { $limit: paginationOptions.limit + 1 }],
    totalCount: [
      {
        $count: "count"
      }
